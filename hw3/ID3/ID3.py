...
class ID3:
    ...
    def entropy(rows: np.ndarray, labels: np.ndarray):
        ...
        for label in counts:
            ...
            impurity -= p_label * math.log(p_label, 2)
        # ========================
    ...
    def info_gain(self, left, left_labels, right, right_labels, current_uncertainty):
        ...
        info_gain_value = current_uncertainty - p * len(left) * self.entropy(left, left_labels) - p * len(right) * self.entropy(right, right_labels)
        # ========================
